...
def main() -> None:
    ...
    with Session(engine) as session:
        while True:
            emails = session.query(Email).filter(Email.processed_at.is_(None)).limit(MAX_MESSAGES_TO_PROCESS).all()
            ...
            # breakpoint()
            session.commit()
            print(f"Marked {len(processed_emails)} emails as processed")
